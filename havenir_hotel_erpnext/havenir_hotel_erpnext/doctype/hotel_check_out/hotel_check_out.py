...
class HotelCheckOut(Document):
    ...
    def validate(self):
        ...
        if (
            ...
            and room_doc.check_in_id == self.check_in_id
        ):
    ...
    def on_submit(self):
        ...
        room_food_order_list = frappe.get_list(
            "Hotel Food Order",
            filters={
                "status": "To Check Out",
                "room": self.room,
        ...
        room_laundry_order_list = frappe.get_list(
            "Hotel Laundry Order",
            filters={
                "status": "To Check Out",
                "room": self.room,
        ...
        for room in check_in_doc.rooms:
            if (
                frappe.db.get_value("Rooms", room.room_no, "room_status")
                == "Checked In"
            ):
                all_checked_out = 0
    ...
    def get_check_in_details(self):
        ...
        return [
            check_in_doc.name,
            check_in_doc.cnic,
            check_in_doc.guest_name,
            check_in_doc.check_in,
            check_in_doc.contact_no,
            check_in_doc.guest_id,
        ]
    ...
    def get_items(self):
        ...
        # Getting Payments
        payment_entry_list = []
        room_payment_entry_list = frappe.get_list(
            "Hotel Payment Entry",
            filters={
                "check_in_id": self.check_in_id,
                "docstatus": 1,
    ...
    if all_checked_out == 1 or self.customer != "Hotel Walk In Customer":
        ...
        if create_walk_in_invoice == 1:
            ...
            if self.total_pos_charges - self.total_payments > 0:
                payment_doc = frappe.new_doc("Hotel Payment Entry")
                payment_doc.room = self.room
                payment_doc.amount_paid = (
                    self.total_pos_charges - self.total_payments - self.discount
                )
                payment_doc.guest_id = self.guest_id
                payment_doc.check_in_id = self.check_in_id
                payment_doc.guest_name = self.guest_name
        ...
        check_out_list = frappe.get_list(
            "Hotel Check Out",
            filters={
                "docstatus": 1,
                "check_in_id": self.check_in_id,
